...
function Analytics:setCaptureUncaughtException(enabled)
	if analytics_plugin ~= nil then
	    analytics_plugin:setCaptureUncaughtException(enabled)
	end
end
...
function Analytics:setAccount(currentUser)
	if analytics_plugin ~= nil then
		if analytics_plugin:isFunctionSupported("setAccount") then
			local paramMap = {
					Account_Id = currentUser.userId,
					Account_Name = currentUser.nickName,
					Account_Type = tostring(AccountType.REGISTED),
					Account_Level = "1",
					Account_Age = "1",
					Account_Operate = tostring(AccountOperate.LOGIN),
					Account_Gender = tostring(AccountGender.UNKNOWN),
					Server_Id = "1",
				}
			if currentUser.gender == '男' then
				paramMap.Account_Gender = tostring(AccountGender.MALE)
			else
				paramMap.Account_Gender = tostring(AccountGender.FEMALE)
			end

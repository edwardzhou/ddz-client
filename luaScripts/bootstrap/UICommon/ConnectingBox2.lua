...
local ConnectingBox2 = class('ConnectingBox2')
...
function ConnectingBox2.extend(target, ...)
  local t = tolua.getpeer(target)
  if not t then
    t = {}
    tolua.setpeer(target, t)
  end
  setmetatable(t, ConnectingBox2)
...
function ConnectingBox2:ctor(params)
  self.showing = false
  self.closeOnTouch = true
  self:init()
end

function ConnectingBox2:init()
  local this = self
  ...
  local uiRoot = cc.CSLoader:createNode('NetworkConnectingLayer2.csb')
  ...
  self:enableButton(self.ButtonNetworkSetup, true)
  self:enableButton(self.ButtonQuit, true)
  self:enableButton(self.ButtonTryAgain, false)

  -- self.LabelMsg:setPosition(260, 0)
end

function ConnectingBox2:enableButton(btn, enabled)
  if enabled == nil then
    enabled = true
  end
...
function ConnectingBox2:doShow(params)
  local this = self
  this.retries = 1
...
function ConnectingBox2:close(fadeOut)
  local this = self
  this.showing = false
...
function ConnectingBox2:initKeypadHandler()
  local this = self
  local function onKeyReleased(keyCode, event)
    if this.showing then
      print('[ConnectingBox2 - onKeyReleased]')
...
function ConnectingBox2:PanelRoot_onClicked(sender, eventType)
  if self.closeOnTouch then
    self:close()
  end
end

function ConnectingBox2:setCurrentRetries(retries)
  local this = self
  local rect, size, pos
  pos = cc.p(260, 40)
  this.retries = retries

  -- print('[ConnectingBox2:setCurrentRetries] --------------', retries)
  -- cclog(debug.traceback())
  -- print('[ConnectingBox2:setCurrentRetries] --------------', retries)
  this:enableButton(this.ButtonTryAgain, false)
...
function ConnectingBox2:setFailure()
  local this = self
  local rect, size, pos
  ...
  -- this.PanelOption:setVisible(true)
  this:enableButton(this.ButtonTryAgain, true)
end
...
function ConnectingBox2:ButtonTryAgain_onClicked(sender, event)
  local this = self
...
local function showConnectingBox2(container, params)

  if container.ConnectingBox == nil then
    local layer = cc.Layer:create()
    local msgBox = ConnectingBox2.extend(layer)
...
local function hideConnectingBox2(container, fadeOut)
  if container.ConnectingBox and container.ConnectingBox.showing then
    ...
    --container.ConnectingBox2 = nil
  end
end

return {
  showConnectingBox = showConnectingBox2,
  hideConnectingBox = hideConnectingBox2
}